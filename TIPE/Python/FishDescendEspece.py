...
def cout_grad(X, Z, theta):
    S = sigmoid(X.dot(theta))
    J = -sum(Z * np.log(S) + (1 - Z) * np.log(1 - S)) / m
    grad = (S - Z).dot(X) / m
    print(J)
    return J * grad

def descente(X, Z, alpha = 0.6, iter = 150, d = -1): # maximise quelle efficacité ?
    #oldTheta = np.zeros((pixelNumber)) # for not commencé à monte # WTF logical
    i = 0
    theta = np.zeros((pixelNumber))
    oldEfficiency = quickEfficacite(X, Z, theta)
    efficiency = oldEfficiency
    while oldEfficiency <= efficiency and efficiency < 1:
        oldEfficiency = efficiency
        theta -= cout_grad(X, Z, theta)
        efficiency = quickEfficacite(X, Z, theta)
...
def descente_tout(X, Y, alpha = 0.6, iter = 150):
    thetatotal = [descente(X, prepY(Y, d), alpha, iter, d) for d in range(speciesNumber)] # descente tant que efficacite de l'espère donné augmente
    return thetatotal
